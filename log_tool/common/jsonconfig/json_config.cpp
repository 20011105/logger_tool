...
#define DYNAMIC_CONFIG_PATH "/home/sys/configs/dynamic.json"//优先级配置文件
...
bool system_config::check_configfile(const std::string &ipaddr)//遍历系统网络接口查找ip是否存在
{
...
bool system_config::dynamic_init(const char* path)
{
    ...
    /*Json::CharReaderBuilder 对象，用于构建 JSON 读取器。*/
    ...
    try
    {   //尝试设置不同设备的日志优先级；打开json文件并读取默认系统配置信息
...
bool system_config::init(const char* path)
{
    ...
    default_init();//初始化前先默认初始化：复位操作应该可以和这里有关系
    ...
    builder["collectComments"] = true;
    JSONCPP_STRING errs;
    if (!parseFromStream(builder, ifs, &root, &errs)) {//从指定路径中读取系统参数并存到root变量
        std::cout << errs << std::endl;
        return false;
    }
    ...
    try
    {
        ...
        parameter_.ipaddress                        = root["ipaddress"].asString();//读取后后面会检查ip
        parameter_.mqtt_host                        = root["mqtt_host"].asString();//192.168.137.1
        parameter_.mqtt_port                        = root["mqtt_port"].asInt();//1883
    ...
    if (dynamic_init(DYNAMIC_CONFIG_PATH) != true)//动态初始化用于调整日至的输出优先级
    {
        return false;
    }
    
    return check_configfile(parameter_.ipaddress);//最后检查读取的ip是否有效
}
...
void system_config::save_configfile()
{
    ...
    std::ofstream fstr;
    fstr.open(file_path_.c_str(), std::ios::out);//存到系统配置类中定义的file_path_路径下
    fstr.write(s.c_str(), s.size());
    fstr.close();
}
